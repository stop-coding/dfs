...
#define LOG_SWITCH_INIT (1<<CAFS_LOG_LEVEL_E_MAX)
...
//syslog
#define SYSLOG_SWICH_ENABLE (1<<(CAFS_LOG_LEVEL_E_MAX + 1))
#define SYSLOG_SWICH_FILE  "on_syslog"

//fprintf
#define FPRINTF_SWICH_ENABLE (1<<(CAFS_LOG_LEVEL_E_MAX + 2))
#define FPRINTF_SWICH_FILE "on_fprintf"

//本地文件
#define LOG_FILE_ENABLE (1<<(CAFS_LOG_LEVEL_E_MAX + 3))
...
static void set_log_switch(const char *module, int32_t *log_enabale)
{
    ...
    if(access(buf, F_OK) == 0){
        *log_enabale = (*log_enabale)|(1<<CAFS_LOG_LEVEL_E_TRACE);
    }
    snprintf(buf, sizeof(buf), "/run/%s/%s", module, LOG_DEBUG_SWICH_FILE);
    if(access(buf, F_OK) == 0){
        *log_enabale = (*log_enabale)|(1<<CAFS_LOG_LEVEL_E_DEBUG);
    }
    snprintf(buf, sizeof(buf), "/run/%s/%s", module, LOG_INFO_SWICH_FILE);
    if(access(buf, F_OK) == 0){
        *log_enabale = (*log_enabale)|(1<<CAFS_LOG_LEVEL_E_INFO);
    }
    snprintf(buf, sizeof(buf), "/run/%s/%s", module, LOG_ERROR_SWICH_FILE);
    if(access(buf, F_OK) != 0){
        *log_enabale = (*log_enabale)|(1<<CAFS_LOG_LEVEL_E_ERROR);
    }
    snprintf(buf, sizeof(buf), "/run/%s/%s", module, LOG_FATAL_SWICH_FILE);
    if(access(buf, F_OK) != 0){
        *log_enabale = (*log_enabale)|(1<<CAFS_LOG_LEVEL_E_FATAL);
    }
...
void cafs_vlog(enum cafs_log_level level, const char *module, const char *file,unsigned line, const char *function, const char *fmt, ...)
{
	...
	static const char * const level_str[CAFS_LOG_LEVEL_E_MAX] = {
	...
	if(level <= CAFS_LOG_LEVEL_E_ERROR) {
